...
public class Converter {
    ...
    private String ConvertTens(int decimal, boolean isDecimal){
        if (decimal == 0){
            return "Zero";
        } else if (decimal < 10){
            if (String.valueOf(decimal).length() == 1 && isDecimal == true){
                return numMap.getTensString(decimal % 10);
            } else {
    ...
    private String ConvertBase(int base){
        ...
        if (base == 0){
            ...
            result = ConvertTens(base, false);
        } else if (base < 1000){
            String hundreds = numMap.getOnesString(base/100);
            String trailingResult = ConvertTens(base % 100, false);
    ...
    public void Convert(){
        ...
        Split();
        decimalStr = ConvertTens(decimal, true);
